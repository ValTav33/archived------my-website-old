...
| Pricing on site | **A `/pricing` page explains the model — fixed build fee agreed before work starts, optional monthly support, and what moves the number — with no figures.** The FAQ's money answer links to it. Figures are still not on the site; when they exist they drop into one section of that page. |
...
| Deliverables & accounts | **Nothing on the site promises that accounts, code or repositories transfer to the client.** Packages differ, and which side administers domain/hosting is the client's choice based on how much time they want to spend on it. What the site does promise is that delivery and ownership are agreed **in writing, per project, before work starts** — true of every package. |
| Technology names | **Not in visitor-facing copy, anywhere.** A shop owner cares about the outcome, not the toolchain. `/privacy` is the sole exception and a required one: a privacy policy must identify its processors. A tools page is deferred, not rejected. |
...
/pricing                Πώς τιμολογούμε — the model, no figures    [built Phase 3.5]
...
| Rule | Decision |
|---|---|
| Palette | Monochrome. Emerald `#10B981` for status dots **only** — no eyebrows, no labels, no icons. **Monochrome locks the hue, not the brightness:** white and near-white carry no hue, so a light surface (`paper` / `graphite`) is inside this rule, not an exception to it. Used deliberately and rarely. **One documented exception:** the form's error state ships red, and has since Phase 0 — see §14, 2026-09-15. |
...
| Motion | Moderate: scroll reveals, scroll-spy nav, hover states. **`position: sticky` choreography is permitted** — the browser scrolls normally and an element stays put. **Scroll-jacking — intercepting or retiming the visitor's scroll — remains banned,** as do parallax and autoplay video. Maximum 1–2 animated elements per viewport, `transform`/`opacity` only. |
| Jargon | `SYS.ENG`, `SYSTEMS OPERATIONAL • LATENCY: NORMAL`, "Protocols" as a heading — **all removed.** Plain Greek. |
| Banned | **Coloured** gradients, glow blobs, Web3/gamer aesthetics, full-screen video, stock photography of handshakes or generic offices. Neutral luminance gradients **are** permitted, for depth, glass edge-light and section transitions. |
| Particles | Permitted **only** under all of: sparse, slow, hero section only, `transform`/`opacity` only, paused off-screen, absent under `prefers-reduced-motion`, and measured — they are the first thing cut if Performance drops below the §6 budget. |
| Glass | The signature material. It needs three things or it reads as a grey box: a translucent fill, `backdrop-filter` (with the `-webkit-` prefix, for Safari), and a **rim light on all four edges** — lit top, faint sides, near-dark bottom, plus a trace of inner bloom. A single top highlight is the common shorthand and is not enough. It needs a fourth thing that is not CSS: something bright behind it to refract. **`backdrop-filter: url(#svg-filter)` is banned** — `CSS.supports` returns false for the `-webkit-` form, so it renders nothing in Safari, which is the browser on the phones this site is for. |
...
| **4** | **Conversion & Craft** | Turn the homepage into a landing page that closes — depth moves to the page that owns it, the offer stops being a footnote, and there is always a way to act on a phone. Surfaces, type, glass and motion are rebuilt in service of that. | The conversion and craft gates in `docs/phases/PHASE-4-CONVERSION-CRAFT.md` §8, **plus** the unchanged Lighthouse floor: Perf ≥ 95, A11y = 100, BP = 100, SEO = 100 |
...
2. **One accent.** Emerald is for live status dots. The form's error state is
   red and always has been — that is the only other chromatic value, it is
   documented rather than permitted to spread, and nothing joins it.
...
9. **Two type ramps, never mixed.** `ink-*` is for dark surfaces, `graphite-*`
   for light ones. An `ink-*` value on `paper` is how a light band ends up
   with 4% contrast text. Both ramps are verified against the **lightest**
   surface they can land on, not against a mid one.
10. **An elevation step must be visible.** Before S4.0 the surface ramp spanned
    1.00–1.18 against the page and a card sat at 1.04:1 — seven named tokens
    the eye read as one. If a new surface is not distinguishable from its
    neighbour at arm's length on a phone, it is not a surface.
...
| 2026-09-14 | **Phase 3.5 inserted between Phases 3 and 4: a content truth pass.** Five corrections Val gave after reading the live site — the surname was misspelled, the site promised account and code transfer that does not hold across his packages, technology names were everywhere, the FAQ set was wrong, and the `/about` copy opened with a sentence that did not mean anything. Its own phase rather than Phase 4's first slices, because Phase 4's gate is a performance measurement and mixing copy into that PR makes a scroll-reveal regression indistinguishable from a sentence change. | Phase 4 was next | Val's call. Truth outranks polish even when the polish is what he complained about. |
| 2026-09-14 | **No promise of account or code transfer, anywhere (§2.2 updated).** Val's rule was *don't mention code ownership unless every developer does it* — and they do not: copyright commonly stays with the author absent written assignment, agencies that host their clients hand over nothing, and on hosted platforms there is no codebase to hand over. Accounts are reframed from a transfer into **the client's choice**, which is both true of every package and a better sentence. `/terms` states that delivery and ownership are agreed in writing per project rather than going silent under its own «Σε ποιον ανήκει ο κώδικας» heading. | The hero, the footer, the `/websites` h1, `/terms`, `/process`, the FAQ and `/about` all promised transfer | The claim was in **14 files**, not the 8 scoped — it lived in the nominative «δικός σας», which the initial survey grep missed. Lesson recorded: in Greek, search the stem, not an inflected form. |
| 2026-09-14 | **No technology names in visitor-facing copy (§2.2 updated).** `TechStackStrip` deleted, framework and vendor names removed from copy, meta descriptions, OG alt text and the `SERVICE_CATALOG` that feeds `knowsAbout`. **`/privacy` is the sole exception and a required one** — a privacy policy must name its processors, and removing them would break the compliance Phase 3 established. A tools page is deferred, not rejected. | Tool names were a section on the homepage and badges on every case study | Val: a shop owner cares about the result, not what it is built with. **V10 closed as a side effect** — «AI Concierge & Voice/Chat Agents» was in that catalogue *and* in the footer on all eleven routes. |
| 2026-09-14 | **`SITE.personAccusative` exists, because Greek inflects.** One name constant cannot serve both «ο Βαλσάμης Ταβλίκος» and «με τον Βαλσάμη Ταβλίκο»; the nominative shipped into a build in an accusative slot. Rewriting sentences to dodge the case would be writing around the language instead of in it. | One `person` constant | Second time Greek grammar has cost a build — the first was the all-caps tonos in S2.5. Any future name or place constant used in more than one case needs the same treatment. |
| 2026-09-14 | **§11.6's solo-operator answer no longer rests on code ownership.** It now answers from what holds in every package: the terms are agreed in writing before anything is built, so what the client ends up with is never a surprise. It lives on `/faq` rather than the homepage, and §11.6 says *in the FAQ* — `/faq` is the FAQ. | The answer promised the repository from day one | D2 removed its foundation. The objection is still §12's silent deal-killer, so it is answered, not dropped. |
| 2026-09-15 | **Phase 4's objective is conversion, not craft. §3's row is retitled «Conversion & Craft».** Val, after reviewing the live site: *"στόχος του landing page είναι το conversion rate. Πάντα"* — and everything analytical belongs on the page that owns it. The first rewrite of the Phase 4 spec had nine slices and **not one sentence about how a visitor becomes a lead**; it was a plan about looking better. The measured case for the correction: the largest block on the homepage is «Ενδεικτικές Αρχιτεκτονικές» at **3.4 phone screens describing systems built for nobody**, while the proof strip — the only credibility on the page — is the smallest at 0.6. A phone visitor scrolls **~9,000px with no CTA in reach**. §2.3 has said *"Homepage carries highlights only"* since v1.0; it was simply never applied to the showcase. | §3: "Scroll reveals, scroll-spy nav, real bento hierarchy, ESLint/CI", gated on three Lighthouse numbers | A site can score 95/100/100 and still be small grey text on a flat black page. The old gate tested nothing Val complained about. The new gate in the phase spec measures ways-to-act, the longest CTA-free stretch, whether the offer appears above the form, text volume, type floor and monospace share — and keeps Lighthouse as a floor rather than as the goal. |
| 2026-09-15 | **The design is unlocked; the palette is not.** Val authorised reopening §2.4 with one condition — *«τα χρώματα να μείνουν τα ίδια»*. Four rules move: `position: sticky` choreography becomes permitted (scroll-jacking and parallax stay banned), "gradients" narrows to "**coloured** gradients" so neutral luminance depth is allowed, particles gain an explicit permission with six conditions attached, and glass is written down as the signature material with its three CSS requirements. The font lock is confirmed unchanged and clarified: it names **which** fonts, never the proportion — monospace was **61 of 108** text elements on the mobile homepage, including every form label, and drops to ≤20%. | §2.4 and §10 were final; CLAUDE.md said not to touch palette, fonts, spacing or animation library | The unlock is Val's call and is recorded rather than assumed. The palette staying put is the constraint that made the light-surface decision below necessary rather than optional. |
| 2026-09-15 | **Monochrome locks the hue, not the brightness — light surfaces are inside §2.4, not an exception to it.** White carries no hue, so a near-white band is monochrome by definition. `paper` / `graphite` / `rule` tokens added, with their own AA-verified type ramp. **Measured justification:** apple.com alternates adjacent homepage tiles between `#000000` and `#F5F5F7` — about **19:1**. Two adjacent near-black bands physically top out around **1.4:1**, so no amount of dark-on-dark striping reaches what Val was pointing at; `paper` against the page is **18.4:1**. Apple's dark sections are carried by bright content inside them, and this site has **0 images**. | Undefined — §2.4 said "monochrome" and everyone read it as "dark" | The one move that answers "δίχρωμο" without adding a single degree of hue. Used once, for the conversion section — §10.9 adds the rule that the two type ramps are never mixed. |
| 2026-09-15 | **The surface ramp was widened and the ink ramp lifted with it (S4.0).** The ramp spanned **1.00 → 1.18** against the page; a card sat at **1.04:1**. Twelve named colours rendered as roughly three perceptible values — the whole "two-colour thing". It now spans 1.00 → 1.63 on the same cool-slate axis. The ink ramp had to move too: `ink-ghost` fell to **4.07:1** on the new card surface, below AA. Every text token is now solved against `obsidian-700`, the **lightest** surface in the system and therefore the worst case, which is what §10.9 formalises. Type: **89 of 108** text elements were 14px or smaller and **59 sat on the 12px floor**; the bottom of the scale moves up (`base` 16→17, matching apple.com's body size) and `lg` closes the gap to `xl`. | Seven surface tokens and five ink tokens chosen by eye; `base` 16px | Solved for, not picked. The ratios in the config comments are the arithmetic, so a later change can be checked rather than re-guessed. |
| 2026-09-15 | **The form's error red is documented rather than removed.** §10.2 said "Emerald… nothing else, ever" while `AuditForm` has shipped `border-red-500/25` and `text-red-300` since Phase 0. The rule now states that the error state is the one other chromatic value, it is documented so it does not spread, and nothing joins it. | §10.2 claimed a purity the code did not have | A design rule that the codebase already violates is not a rule, it is a note nobody checked. Making the document honest costs nothing and removes the argument that amending §2.4 breaks its integrity. |
| 2026-09-15 | **Phase 4 ships no photography; animated elements do that job.** Val was given a seven-shot list he could take on a phone — portrait, workspace, three client screenshots — and declined it: *"όχι τέτοιες φωτογραφίες τότε. Ίσως βάλουμε animated στοιχεία."* Consequences recorded so a later session does not quietly reopen it: glass has nothing bright to refract except the light band and the animated elements, so **both are load-bearing rather than decorative**; every animated element stays inside §8 — abstract, labelled «ενδεικτικό», and never drawn to resemble a screenshot of delivered work; `PORTRAIT` stays `null` and **V14 stays open**. The showcase stays on the homepage but shrinks to three one-line cards, its bodies moving to `/websites` and `/automations` — Val's call, against the recommendation to remove it entirely. | Phase 5 assumed real assets would arrive; the first Phase 4 draft assumed a portrait and screenshots | A harder brief, taken deliberately. The day a real screenshot exists it replaces an animated placeholder rather than being added beside it. |
| 2026-09-15 | **The 21st.dev liquid-glass component is not adopted; its rim-light technique is.** Val supplied a shadcn component for Liquid Glass. Rejected on five measured grounds, not on taste: its actual effect is `backdrop-filter: url("#container-glass")` and **`CSS.supports('-webkit-backdrop-filter','url()')` returns false** — it renders nothing in Safari, the browser on the phones this site targets; it is written in **Tailwind v4 syntax** (`bg-linear-to-t`, `inset-shadow-2xs`, `size-4`, `has-[>svg]:`) against our **3.4.17**; it references **shadcn design tokens** (`bg-primary`, `ring-ring`, `bg-accent`, `bg-destructive`) of which this project defines **none**; it needs `@radix-ui/react-slot` and `class-variance-authority`, two dependencies for one button and two more rows under §13's monthly cadence; and its `MetalButton` variants are gold and bronze **coloured** gradients, banned by §2.4 and by Val's own *«τα χρώματα να μείνουν τα ίδια»*. Its integration notes also instruct filling image slots with Unsplash stock photography, against both §2.4 and Val's 2026-09-15 decision. **What was taken** is the one genuinely good idea in it: a layered inset box-shadow rim rather than a single top highlight — pure CSS, Safari-safe, free. Tuned in the browser against a heavier variant, which read as a plastic capsule. | `.glass` shipped with one `inset 0 1px 0` highlight | A pasted component is a design decision arriving as a diff. This one would have shipped an effect that is invisible to the target audience and a palette the project has twice decided against. |
| 2026-09-15 | **Phase 4's homepage length gate is withdrawn; "how far can you scroll without being able to act" becomes the load-bearing one.** Val, asked whether to drop Process and About to reach ≤ 7 phone screens: *«κρατάμε αυτές που έχει και βάζουμε περισσότερες ενότητες στο μέλλον.»* **The measurement that produced the question:** S4.1 cut the showcase from 3.4 phone screens to 1.7 and the page still landed at 12.1, because on a 375px viewport every card grid stacks into a column — four stacked groups at 230–280px per card is ~2,500px before a single paragraph, and S4.0's larger type had added ~600px. Tightening prose could not have closed it; only deleting sections could, and that is a §2.3 structure decision a session does not get to make alone. | Phase 4 spec §8: ≤ 600 words, ≤ 7 phone screens | A cap on total length fights the stated plan for the page. On a page that grows, the defect is a visitor with nothing to act on — **~9,000px of the current page has no CTA in reach** — so that is what the gate measures, and every future section inherits the fix rather than re-opening the question. Two density rules join it: every section links out or carries a CTA, and no claim renders twice on the page. |
