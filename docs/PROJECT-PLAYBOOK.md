...
| Glass | The signature material. It needs three things or it reads as a grey box: a translucent fill, `backdrop-filter` (with the `-webkit-` prefix, for Safari), and the **inset top edge-light**. It needs a fourth thing that is not CSS: something bright behind it to refract. |
